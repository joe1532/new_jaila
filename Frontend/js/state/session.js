...
const TEST_CHAT_SESSION_KEY = "jaila_test_chat_session_id";
...
export function getOrCreateTestChatSessionId() {
  try {
    const existing = localStorage.getItem(TEST_CHAT_SESSION_KEY);
    if (existing && /^[a-zA-Z0-9_-]{8,80}$/.test(existing)) {
      return existing;
    }
    const created = createSessionId();
    localStorage.setItem(TEST_CHAT_SESSION_KEY, created);
    return created;
  } catch (_err) {
    return createSessionId();
  }
...
export function resetTestChatSessionId() {
  const created = createSessionId();
  try {
    localStorage.setItem(TEST_CHAT_SESSION_KEY, created);
  } catch (_err) {
...
export function setTestChatSessionId(sessionId) {
  const value = String(sessionId || "").trim();
  if (!/^[a-zA-Z0-9_-]{8,80}$/.test(value)) {
    return null;
  }
  try {
    localStorage.setItem(TEST_CHAT_SESSION_KEY, value);
  } catch (_err) {
    // Ignorer localStorage-fejl.
  }
  return value;
